  ...
  try {
    const [result] = await db.query(`
      INSERT INTO metaltype (
        metal_name, hsn_code, description, default_purity,
        default_purity_for_rate_entry, default_purity_for_old_metal, default_issue_purity
      ) VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [
      data.metal_name,
      data.hsn_code,
...
router.put('/metaltype/:id', async (req, res) => {
  ...
  try {
    const [result] = await db.query(`
      UPDATE metaltype
      SET metal_name = ?, hsn_code = ?, description = ?, 
          default_purity = ?, default_purity_for_rate_entry = ?, 
          default_purity_for_old_metal = ?, default_issue_purity = ?
      WHERE metal_type_id = ?
    `, [
      data.metal_name,
      data.hsn_code,
